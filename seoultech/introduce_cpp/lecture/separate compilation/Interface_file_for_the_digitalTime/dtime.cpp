...
DigitalTime::DigitalTime(int theHour, int theMinute){
  if(theHour <0 || theHour >24 || theMinute <0 || theMinute >59){
    cout <<"Illegal argument to digitalTime constrcutor.";
    exit(1);
  }
  else{
    hour = theHour;
...
istream& operator >>(istream& ins, DigitalTime& theObject){
  ...
  DigitalTime::readMinut(theObject.minute);
  return ins;
}
...
void DigitalTime::readMinut(int& theMinute){
  char c1, c2;
  cin >> c1 >>c2;
...
void DigitalTime::readHour(int& theHour){
  ...
  if(theHour == 24) theHour = 0; //standardize midngith as 0:00
